...
def comment_del(id=None):
    comment = User.query.get_or_404(int(id))
...
def role_add():
    ...
    if form.validate_on_submit():
        ...
        role = Role(
            name=data["name"],
            auths=','.join(map(lambda v:str(v), data["auths"]))  # 这里将列表转换为字符串
        )
        db.session.add(role)
        db.session.commit()
        flash("角色添加成功","ok")
